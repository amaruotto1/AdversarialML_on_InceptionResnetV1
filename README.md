...
* **Attack Generation:** Create untargeted and targeted adversarial examples bounded spatially by the L_infinity norm with a maximum perturbation budget of epsilon=0.10.
...
| :--- | :--- | :--- | :--- |
| **NN1** | Inception-ResNet-v1 | Surrogate Model (White-Box) | 84.50% |
| **NN2** | ResNet-50 | Target Model (Black-Box) | 88.70% |

---

## Evaluated Attacks
The entire suite of attacks was implemented leveraging the Adversarial Robustness Toolbox (ART).
...
* **Carlini & Wagner (L_infinity):** Advanced optimization algorithm on logits to minimize distortion while guaranteeing evasion (fixed via monkey patching for the targeted variant).

---

## Defense Strategies
1. **Input Transformation (JPEG Compression):** Tested as a spatial low-pass filter, this defense proved largely ineffective against optimized iterative attacks like PGD and BIM. This demonstrates that modern adversarial perturbations do not reside exclusively in the high-frequency spectrum.
2. **Adversarial Detector in Latent Space (Zero Trust Architecture):** A Random Forest classifier was trained to discriminate 512-dimensional embeddings extracted from the neural network. The optimal operational configuration (trained on PGD+BIM+CW) reduced the residual attack success rate to almost 0%. However, this highlighted an inevitable usability cost, generating a False Positive Rate (FPR) of 26.5% on legitimate users.
